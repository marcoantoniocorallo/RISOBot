...
def listener(msgs):
    ...
    for m in msgs:
        ...
        if m.content_type=='text':
            ...
            if text in start:
                bot.send_message(chatid,welcomeMessage)
...
def handle_docs(doc,user):
    ...
    users[user].plan = inPath+str(users[user].userid)+".pdf"
    ...
    if checkCorrectness(users[user].plan):
        ...
        bot.send_message(users[user].chatid,errorFileMsg)
